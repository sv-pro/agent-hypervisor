"""
firewall.py — Provenance-aware tool execution firewall.

The firewall sits between an agent and actual tool execution.
...
Usage:
    ...
    from agent_hypervisor.models import ToolCall, ValueRef
...
class ProvenanceFirewall:
    ...
    Instantiate with a task manifest dict or load directly from YAML:
    ...
    @classmethod
    def from_manifest(
        cls, path: str | Path, protection_enabled: bool = True
    ) -> "ProvenanceFirewall":
        """Load a ProvenanceFirewall from a YAML task manifest file."""
    ...
    def check(self, call: ToolCall, registry: dict[str, ValueRef]) -> Decision:
        """
        Evaluate a ToolCall and return a Decision (allow / deny / ask).

        Args:
            call:     The proposed tool invocation.
            registry: All ValueRefs currently in scope, keyed by id.

        Returns:
            Decision with verdict, reason, and violated_rules if denied.
        """
    ...
    def _check_send_email(
        ...
        least_trusted_prov = least_trusted(chain_prov)
    ...
    def _check_side_effect(
        ...
        return self._allow(
            call, f"Tool '{call.tool}' granted with clean provenance", registry=registry
        )
    ...
    def _find_declared_recipient_source(self, chain: list[ValueRef]) -> str | None:
        ...
        for ref in chain:
            ...
            if (
                Role.recipient_source in ref.roles
                and ref.provenance == ProvenanceClass.user_declared
            ):
                return ref.source_label
        return None

    def _arg_provenance(
        self, call: ToolCall, registry: dict[str, ValueRef]
    ) -> dict[str, str]:
        return {k: provenance_summary(v, registry) for k, v in call.args.items()}
